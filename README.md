...
**Current release: v57 · PROFIT READINESS GATE · AUDITED**
...
Use **`DEPLOY_V57.md`** as the authoritative deployment guide. Older deployment documents are retained under `docs/legacy/` only for historical reference.
...
The v57 test gate covers syntax, API security/origin/rate-limit behavior, deterministic runtime fixtures, the Verdict Explainer fixture, the dedicated Profit Readiness state-machine fixture, the inherited 62-finding remediation matrix and retained wide-terminal layout invariants.
...
## v55 · WIDE TERMINAL UI

v55 keeps Engine Contract 54.1 and the hardened v54 research/security paths, but rebuilds the desktop shell to use the full monitor width. The accidental 900 px body cap and the desktop main-width cap are removed, the Dashboard becomes a 24-column terminal grid (30 columns on ultrawide displays), the sidebar is compacted, the chart grows substantially, and the major research panels are arranged in parallel columns. Below 980 px the existing mobile navigation/layout path is retained. No new provider or API key is required.


## v56 · VERDICT EXPLAINER

v56 keeps the full-width v55 terminal and hardened Engine Contract 54.1, but makes Master Verdict explicit and actionable. The top decision block now states the base bias, final executable research verdict, blocker/warning state, directional agreement, calibrated base-direction probability, weighted consensus, module coverage and data quality. It separately lists the strongest supporting evidence, strongest opposing evidence and exact conditions required to unlock a blocked WAIT. When WAIT still has a LONG/SHORT candidate trade map, the map is clearly marked INACTIV. No new provider/API key or live-money execution route is introduced.


## v57 · PROFIT READINESS GATE

v57 adds a conservative evidence gate above the existing research stack. For the selected market/source it reports `NOT READY`, `PAPER READY`, or `SMALL LIVE READY` from cost-aware resolved outcomes, Forward evidence, completed Paper executions, recent stability, regime diversity, current-setup OOS, model/drift state, Data Quality and explicit post-deploy provider confirmation.

`SMALL LIVE READY` is intentionally **not** a prediction of profit and not a trade recommendation. It means only that the configured evidence checklist passed. A live trade still requires its own Master Verdict and risk controls.

No new API provider, secret, D1 schema or live-order route is required by v57. Engine Contract remains 54.1 and data schema remains 54.
