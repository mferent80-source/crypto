...
export async function onRequestGet({request}){
  ...
  if(type==="health")return ok({ok:true,service:"crypto-radar",version:"v31"});
  if(type==="pionex_symbols"){
    try{return ok(await j(`${PIONEX}/api/v1/common/symbols?type=SPOT`))}catch(e){return new Response(JSON.stringify({error:"Pionex symbols unavailable",detail:e.message}),{status:502,headers:H})}
  }
  if(type==="pionex_tickers"){
    try{return ok(await j(`${PIONEX}/api/v1/market/tickers?type=SPOT`))}catch(e){return new Response(JSON.stringify({error:"Pionex tickers unavailable",detail:e.message}),{status:502,headers:H})}
  }
  if(type==="pionex_klines"){
    ...
    try{return ok(await j(`${PIONEX}/api/v1/market/klines?symbol=${encodeURIComponent(ps)}&interval=${encodeURIComponent(interval)}&limit=${limit}`))}catch(e){return new Response(JSON.stringify({error:"Pionex klines unavailable",detail:e.message}),{status:502,headers:H})}
  }
  if(type==="pionex_trades"){
    ...
    try{return ok(await j(`${PIONEX}/api/v1/market/trades?symbol=${encodeURIComponent(ps)}&limit=${limit}`))}catch(e){return new Response(JSON.stringify({error:"Pionex trades unavailable",detail:e.message}),{status:502,headers:H})}
  }
  if(type==="pionex_depth"){
    ...
    try{return ok(await j(`${PIONEX}/api/v1/market/depth?symbol=${encodeURIComponent(ps)}&limit=${limit}`))}catch(e){return new Response(JSON.stringify({error:"Pionex depth unavailable",detail:e.message}),{status:502,headers:H})}
  }
