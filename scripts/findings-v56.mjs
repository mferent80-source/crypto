...
const data=JSON.parse(await readFile(new URL('../AUDIT_V56_FINDINGS.json',import.meta.url),'utf8'));
...
console.log(`V56_FINDINGS_MATRIX_PASS total=${data.findings.length} fixed=${data.counts.fixed} mitigated=${data.counts.mitigated} open=${data.counts.open}`);
