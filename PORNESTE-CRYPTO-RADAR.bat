...
rem [ps:sanatate]
%PORNIRE% powershell -NoProfile -Command "$u = 'http://127.0.0.1:8788/api/market?type=health'; for ($i = 0; $i -lt 90; $i++) { try { $r = Invoke-RestMethod -Uri $u -TimeoutSec 3 -ErrorAction Stop; if ($r -and $r.ok -eq $true -and $r.service -eq 'crypto-radar') { try { Start-Process -FilePath 'node' -ArgumentList 'scripts\colector.mjs' -WorkingDirectory (Get-Location).Path -WindowStyle Hidden -ErrorAction Stop } catch { Write-Host '  [ATENTIE] Colectorul de alerte nu a pornit (node lipsa?).' }; & powershell -NoProfile -ExecutionPolicy Bypass -File 'scripts\inchide-ferestre.ps1' | Out-Host; $edge = @('C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe', 'C:\Program Files\Microsoft\Edge\Application\msedge.exe') | Where-Object { Test-Path $_ } | Select-Object -First 1; if ($edge) { Start-Process -FilePath $edge -ArgumentList '--app=http://127.0.0.1:8788/' } else { Start-Process 'http://127.0.0.1:8788/' }; exit 0 } } catch {}; Start-Sleep -Seconds 2 }; Write-Host ''; Write-Host '  [ATENTIE] Pe 127.0.0.1:8788 nu raspunde Crypto Radar (am cerut /api/market?type=health).'; Write-Host '      Nu deschid browserul pe un raspuns care nu e al aplicatiei.'; exit 1"
set "RC=%ERRORLEVEL%"
